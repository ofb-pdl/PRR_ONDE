...
-   La couleur des symboles correspond à la situation des observations au cours de ces dernières campagnes ;
-   La taille des symboles est proportionnelle à la fréquence des assecs sur les chroniques de données des campagnes usuelles ;
...
-   **Typologie départementale** : [**écoulement visible acceptable**]{style="color: #0570b0;"} ; [**écoulement visible faible**]{style="color: #FFFF80;"} ; [**écoulement non visible**]{style="color: #feb24c;"} et [**assec**]{style="color: #e31a1c;"}.
...
### **Évolution des conditions d'écoulements lors des campagnes usuelles par mois (stations encore actives)** {.tabset}

<br>

```{r, results='asis', fig.height=5.5, fig.width=9, align='center'}
for (m in names(graphs_mois)) {

  cat(paste0("\n#### ", m, " {.tabset}\n"))

  cat("\n##### Typologie nationale\n")
  print(graphs_mois[[m]]$reg_nat)

  cat("\n")

  cat("\n##### Typologie départementale\n")
  print(graphs_mois[[m]]$reg_dep)
...
for (d in conf_dep) {
  if (d %in% df_categ_obs_4mod$code_departement) {
   ...
   cat("\n#### **Évolution des conditions d'écoulement (stations encore actives)** {.tabset}\n")

for (m in names(graphs_mois)) {

  cat(paste0("\n##### ", m, "\n"))

  print(graphs_mois[[m]]$dep[[d]])
...
L'indice ONDE est calculé pour les campagnes (usuelles et complémentaires) où toutes les stations du département ont été prospectées.

<br>

<div style="margin-bottom:20px;">

<button onclick="showPlot('44')" class="dep-btn">
  Loire-Atlantique
</button>

<button onclick="showPlot('49')" class="dep-btn">
  Maine-et-Loire
</button>

<button onclick="showPlot('53')" class="dep-btn">
  Mayenne
</button>

<button onclick="showPlot('72')" class="dep-btn">
  Sarthe
</button>

<button onclick="showPlot('85')" class="dep-btn">
  Vendée
</button>
...
deps <- c(44, 49, 53, 72, 85)

plots <- lapply(deps, function(d){
...
function showPlot(dep) {

  var deps = ["44","49","53","72","85"];

  deps.forEach(function(d) {
    document.getElementById("plot_" + d).style.display = "none";
  });

  document.getElementById("plot_" + dep).style.display = "block";
}
...
Seules les observations des stations encore actives ont été prises en compte. En Sarthe, le réseau de stations a été modifié en 2017. 7 stations suivies depuis 2012 sont toujours suivies aujourd'hui. 25 stations ne sont plus suivies et ont été remplacées par des nouvelles stations en 2017. Cela explique qu'il y ait des données manquantes pour le département.
...
L'indice ONDE est calculé à l'échelle départementale. Il est comparable sur un même référentiel de station, c'est pourquoi il ne peut être calculé que pour les campagnes où toutes les stations ont été prospectées. 
Sa formule de calcul est la suivante : 

**Indice ONDE = (5 x N2 + 10 x N1)/N**

N : nombre total de stations, N1 : nombre de stations en écoulement continu, N2 : nombre de stations en écoulement interrompu (écoulement non visible)

Les assecs ne rapportent pas de point.

**La valorisation présentée ici**

La lecture des données, leur mise en forme et la production des graphiques sont effectués au moyen du logiciel *R* et de plusieurs *packages* (dont `ondetools`, `hubeau`, `tidyverse`, `ggplot2`, `rmarkdown`, `leaflet`, `plotly`, `htmltools` et `mapview`).
